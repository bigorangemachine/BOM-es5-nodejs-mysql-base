...
module.exports = function(mysql, _, utils, merge){
    ...
    var self_init=function(){
            ...
            this.add_schema({'col_name': 'date_stamp', 'is_null': false, 'val_type': 'date', 'key_type': 'updatestamp'});
        };
    ...
    genericDB.prototype.add_schema=function(schemaObj, tableName){
        ...
        new_column=merge(true, new_column, schemaObj);
...
console.log('tableName',tableName,self.constructor.name);
    ...
    genericDB.prototype.escape_val=function(valIn, colName, tableName){
        var self=this,
            do_nothing_list=['string','date'],//should be ['string']
            number_list=['int','float'],
            output='',
            col_data=self.get_column(colName, tableName);

        if(_.indexOf(do_nothing_list, col_data.val_type)!==-1){
            output=valIn;
//        }else if(col_data.val_type==='date'){// this needs more data detection
//            output=valIn;
        }else if(col_data.val_type==='enum'){//I hate enums.... will do if necessary
            throw new Error("Enums currently not built");return false;
        }else if(_.indexOf(number_list, col_data.val_type)!==-1){
            output=valIn;
            if(typeof(valIn)!=='number'){
                output=(col_data.val_type==='float'?parseFloat(valIn):parseInt(valIn));
            }
        }else if(col_data.val_type==='boolean'){
            output=(valIn===true?'1':'0');
        }else{
           throw new Error("Could not escape value '"+escape_val+"' because it is a type '"+col_data.val_type+"'.");return false;
        }
        if(col_data.val_type==='string' && output.length>col_data.size){
            output=output.substr(0, col_data.size);}

        return mysql.escape(output);
    };
    ...
    genericDB.prototype.column_schema_base=function(optsIn){
        ...
        opts.hook_ins={
            'select': (typeof(opts.hook_ins)==='object' && typeof(opts.hook_ins.select)==='function'?opts.hook_ins.select:genericDB.prototype.column_schema_select), //adding to a select part of the query
            'where':(typeof(opts.hook_ins)==='object' && typeof(opts.hook_ins.where)==='function'?opts.hook_ins.where:genericDB.prototype.column_schema_where), //adding to the where part of the query
            'write': (typeof(opts.hook_ins)==='object' && typeof(opts.hook_ins.write)==='function'?opts.hook_ins.write:genericDB.prototype.column_schema_write) //validating a write data set
        };
        return new column_schema(opts);
    };
    genericDB.prototype.where_schema_base=function(){
        return where_obj.schema();
    };
    ...
    genericDB.prototype.column_schema_select=function(colName, tableName, dataObj){
        var self=this,output=[];
    };
    genericDB.prototype.column_schema_where=function(colName, tableName, dataObj){
        var self=this,logical_whitelist=[],output=[];
        logical_whitelist=[];
        self.where_schema_base();
    };
    genericDB.prototype.column_schema_write=function(colName, tableName, dataObj){
        var self=this,output=[];
    };
